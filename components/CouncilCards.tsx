...
const PERSONAS: { key: keyof NonNullable<DossierUi["council"]>; name: string; accent: string }[] = [
  ...
  { key: "quant", name: "QuantAnalyst", accent: "text-sky-400" },
    ...
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-bold text-slate-100">
          ...
          <span className="ml-1 text-xs font-normal text-slate-500">({opinion.confidence})</span>
        </span>
      </div>
      <p className="mt-2 text-xs leading-relaxed text-slate-300">{opinion.thesis}</p>
...
export default function CouncilCards({ council }: { council: NonNullable<DossierUi["council"]> }) {
  ...
  return (
    <section>
      <h2 className="mb-3 text-lg font-semibold text-slate-200">AI council</h2>
