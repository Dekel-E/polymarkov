...
- **Exactly 7 LLM calls per `/api/execute`**: QueryPlanner → SentimentScorer → Bull/Bear/Quant/ResolutionSkeptic (concurrent) → Judge. Fewer on short-circuits (cache hit = 0, meta/out-of-scope = 1, empty sentiment items skips #2). Adding an LLM call to the execute path changes a graded contract — don't do it casually.
